...
//Code Here
var evenNumbers = mixedNumbers.filter(function(val) {
  return val % 2 === 0
})
...
/*numbers
  ...
  Instead of just arrays of , we are going to have array of objects that we want to use map, filter, and reduce with.
...
let orderTotals = orders.map(function(element, i) {
  tax = 1 + element.tax
  total1= element.price * tax 
  return total1
})
...
let bobsTotal = purchases.reduce(function(total, order) {
  price = order.price
  if (order.owner === "Bob") {
    return total + price
  } else {
